...
let { CreateAnUserValidator, validatedResult, ModifyAnUser, ChangePasswordValidator } = require('../utils/validateHandler')
...
router.post("/changepassword", CheckLogin, ChangePasswordValidator, validatedResult, async function (req, res, next) {
  try {
    ...
    let updatedUser = await userController.ChangePassword(req.user._id, oldPassword, newPassword);
    res.send({
